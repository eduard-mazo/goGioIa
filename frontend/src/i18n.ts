// Textos en español para goGioIa. Tono breve, de herramienta interna.
export const t = {
  app: {
    name: 'goGioIa',
    tagline: 'IA LOCAL · OLLAMA',
    title: 'Asistente',
    breadcrumb: 'chat',
  },
  sidebar: {
    newChat: 'Nueva conversación',
    history: 'Historial',
    noHistory: 'Tus conversaciones aparecerán aquí.',
    untitled: 'Conversación nueva',
    deleteChat: 'Eliminar conversación',
    docs: 'Documentos',
    noDocs: 'Adjunta un PDF con el clip para usarlo como contexto.',
    chars: 'caracteres',
    model: 'Modelo',
    ollama: 'Ollama',
    light: 'Modo claro',
    dark: 'Modo oscuro',
    remove: 'Quitar documento',
    collapse: 'Colapsar',
    openMenu: 'Abrir menú',
    closeMenu: 'Cerrar menú',
  },
  status: {
    online: 'Conectado',
    offline: 'Sin conexión',
    checking: 'Comprobando…',
  },
  models: {
    choose: 'Elegir modelo',
    none: 'Sin modelos disponibles',
  },
  chat: {
    empty: 'Escribe un mensaje para empezar.',
    placeholder: 'Escribe un mensaje…',
    stop: 'Detener',
    send: 'Enviar',
    attach: 'Adjuntar PDF',
    you: 'Tú',
    offlineTitle: 'Sin conexión con Ollama',
    offlineBody: (host: string) =>
      `No se pudo contactar el servicio en ${host}. Revisa que Ollama esté activo y vuelve a intentar.`,
    typing: 'Escribiendo',
    error: 'Error',
  },
  doc: {
    uploadFailed: 'No se pudo procesar el PDF',
  },
  rag: {
    toggle: 'Modo asistente (RAG)',
    hint: 'Las respuestas se basan en la base de conocimiento (Oracle 23ai). Sube PDFs desde «Base de conocimiento».',
    sources: 'Fuentes',
    page: 'pág.',
    kbTitle: 'Base de conocimiento',
    kbSubtitle: 'RAG · ORACLE 23AI',
    kbIntro:
      'Sube documentos PDF: se extrae el texto, se trocea y se vectoriza con nomic-embed-text. Cada documento entrena al asistente automáticamente.',
    upload: 'Subir PDF',
    uploading: 'Subiendo…',
    dropHere: 'Arrastra un PDF aquí o haz clic para seleccionarlo',
    documents: 'Documentos',
    empty: 'Aún no hay documentos. Sube el primero para entrenar al asistente.',
    columns: {
      name: 'Documento',
      pages: 'Págs.',
      chunks: 'Chunks',
      size: 'Tamaño',
      status: 'Estado',
      uploaded: 'Subido',
      actions: '',
    },
    status: {
      UPLOADED: 'En cola',
      EXTRACTING: 'Extrayendo',
      CHUNKED: 'Troceado',
      EMBEDDED: 'Entrenado',
      FAILED: 'Error',
    } as Record<string, string>,
    delete: 'Eliminar documento',
    confirmDelete: '¿Eliminar este documento y sus vectores de la base de conocimiento?',
    oracle: 'Oracle 23ai',
    stats: (docs: number, chunks: number) => `${docs} documentos · ${chunks} chunks vectorizados`,
    close: 'Cerrar',
    open: 'Base de conocimiento',
    feedbackUp: 'Respuesta útil',
    feedbackDown: 'Respuesta incorrecta',
    feedbackThanks: 'Gracias por tu valoración',
    searching: 'Consultando la base de conocimiento…',
  },
  ops: {
    open: 'Operaciones RAG',
    title: 'Operaciones RAG',
    subtitle: 'OBSERVABILIDAD · INGESTA · CONSULTAS',
    backToChat: 'Volver al chat',
    refresh: 'Actualizar',
    nav: {
      overview: 'Resumen',
      ingestion: 'Ingesta',
      tokens: 'Tokens y uso',
      retrieval: 'Calidad del retrieval',
      queries: 'Consultas y respuestas',
      models: 'Modelos y Ollama',
      integrity: 'Integridad de datos',
      config: 'Configuración',
    },
    range: { h6: '6 h', h24: '24 h', h72: '3 días', h168: '7 días', label: 'Rango' },
    health: {
      title: 'Salud de componentes',
      app: 'API de la aplicación',
      oracle: 'Oracle 23ai',
      vectorIndex: 'Índice vectorial',
      ollama: 'Endpoint Ollama',
      embedModel: 'Modelo de embeddings',
      genModel: 'Modelo de generación',
      ingestion: 'Pipeline de ingesta',
      queue: 'Cola de ingesta',
      states: {
        healthy: 'Correcto',
        degraded: 'Degradado',
        unavailable: 'No disponible',
        unknown: 'Desconocido',
      } as Record<string, string>,
      cached: 'resultado cacheado',
    },
    tokenSource: {
      ollama: 'Conteo exacto reportado por Ollama',
      estimated: 'Estimación (~4 caracteres por token); no es un conteo del tokenizador real',
      badgeOllama: 'exacto',
      badgeEstimated: 'estimado',
    },
    notInstrumented:
      'Sin instrumentar: no existe dataset dorado de evaluación ni reranking/retrieval léxico en esta aplicación. Estas métricas requieren implementar ese modelo de evaluación; no se muestran valores inventados.',
    empty: 'Sin datos en el rango seleccionado.',
  },
} as const
